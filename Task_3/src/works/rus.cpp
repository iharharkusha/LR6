    ...
    while (*text_ptr != '\0') {
        ...
        if (isLetter(text_ptr)) {
            ...
            if (isLetter(text_ptr)) {
                ...
                unsigned int word_size = (word_end - word_start) / 2;
...
void swapWordsInText(const char* text, char**& ptrs_to_words, unsigned int*& ptrs_to_sizes, unsigned int words_total, unsigned int& temp_indx) {
    for (unsigned int i = 0; i < words_total; i += 2) {
        ...
        //если размеры двух слов разные
            ...
            unsigned int between_sequence_size = getBetweenSequenceSize(text, ptr_word1 + size_word1 * 2, ptr_word2);
            char* between_seq = getBetweenSequence(text, ptr_word1 + size_word1 * 2, ptr_word2, between_sequence_size); 
